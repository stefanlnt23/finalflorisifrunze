...
// Lazy load MongoDB storage to avoid circular dependency
async function loadMongoDBStorage() {
  if (!MongoDBStorage) {
    const module = await import('./mongodb-storage');
    MongoDBStorage = module.MongoDBStorage;
  }
...
export async function createStorage(): Promise<IStorage> {
  const StorageClass = await loadMongoDBStorage();
